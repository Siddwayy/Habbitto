  ...
  function renderTimeSpentContent() {
    if (timeSpentFilter === 'habits') {
      ...
      dailyTotals.forEach((d, i) => {
        ...
        const showInsideTime = !isZero && barHeight >= 20;
        parts.push(`<div class="daily-bar-wrap" title="${fullLabel}: ${displayTime}">
          <div class="daily-bar ${isZero ? 'daily-bar-zero' : ''}" style="height: ${barHeight}px">
            ${showInsideTime ? `<span class="daily-bar-time">${displayTime}</span>` : ''}
          </div>
